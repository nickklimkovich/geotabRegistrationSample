...
document.addEventListener("DOMContentLoaded", function () {
     ...
     */
    changeValidationState = function changeValidationState(el, state) {
     ...
     */
    createDatabaseNameFromCompany = function createDatabaseNameFromCompany(companyName) {
     ...
     */
    checkAvailability = function checkAvailability(databaseName) {
     ...
     * @param companyName
     */
    updateShortDatabase = function updateShortDatabase(companyName) {
     ...
     */
    renderTimeZones = function renderTimeZones() {
        call(host, "GetTimeZones").then(function (timeZones) {
            elTimeZone.innerHTML = timeZones.sort(function (a, b) {
     ...
     * @returns {boolean}
     */
    isFormValid = function isFormValid(values) {
        var isValid = true;
     ...
     */
    createDatabase = function createDatabase(params) {
        if (CONFIG.debug) {
        ...
        createDebugDatabase = function createDebugDatabase() {
     ...
     */
    createClearance = function createClearance(options) {
     ...
     * @returns {object} - options
     */
    setUserDefaults = function setUserDefaults(options) {
     ...
     * @returns {object} - options
     */
    uploadConfigFile = function uploadConfigFile(options) {

        return new Promise(function (resolve, reject) {
            var fileReader = new FileReader(),
                errorHandler = function errorHandler(evt) {
    ...
    importConfig = function importConfig(options) {
    ...
    elSubmit.addEventListener("click", function (evt) {
        ...
        var error = function error(_error) {
            ...
            showError(_error);
            if (_error.name === "CaptchaException") {
        ...
        createDatabase(formValues).then(getUser).then(uploadConfigFile).then(createClearance).then(setUserDefaults).then(importConfig).then(logout).then(redirect)["catch"](error);
    });
